<template> <div>
  ...
  <b-card header="View / Data Auditor Team" header-tag="h4" class="mb-4">
      ...
      <div class="col-md-6">
          ...
          <b-btn @click="createData()" class="btn btn-success btn-md float-right mr-2" v-if="User.accessMenuDB[1].children[1].children[0].selected === true">
      ...
      <template slot="tableHeader">
        <tr class="text-center">
          <th class="wd-50"></th>
          <th>
            <b-input v-model="paramData.search.ads__NoTrans" placeholder="IdAudit" v-on:keyup.enter="getFilters()" />
          </th>
          <th>
          ...
          <th>
            <masked-input type="text" class="form-control" placeholder="Thn-Bln-Tgl"
              v-model="paramData.search.ads__AuditDate"
              v-on:keyup.enter="getFilters()"
              :guide="false"
              :mask="dateYmdMask" />
          </th>
          <th>
            <b-input v-model="paramData.search.ads__HourStart" placeholder="Hour Start" v-on:keyup.enter="getFilters()" />
          </th>
          <th>
            <b-input v-model="paramData.search.ads__HourDone" placeholder="Hour End" v-on:keyup.enter="getFilters()" />
          </th>
          <th>
          ...
          <th></th>
          <th>
            <b-input v-model="paramData.search.emp__Name" placeholder="Lead Auditor" v-on:keyup.enter="getFilters()" />
          </th>
          <th></th>
          <th></th>
          <th></th>
      ...
      <template slot="Path" slot-scope="props">
          <div v-if="props.rowData.Path != ''">
            ...
            @click="viewAttachment(props.rowData, props.rowIndex)" v-if="User.accessMenuDB[1].children[1].children[2].selected === true">
      ...
      <template slot="EmpApproved" slot-scope="props">
        ...
        <template slot="Actions" slot-scope="props">
            <div class="custom-actions" v-if="props.rowData.Actived == 1">
                ...
                @click="onAction('edit-item', props.rowData, props.rowIndex)" v-if="User.accessMenuDB[1].children[1].children[0].selected === true">
                <i class="ion ion-md-create"></i> Edit
                </b-btn>
                <b-btn class="btn btn-outline-danger btn-sm mr-1 mt-1"
                @click="onAction('delete-item', props.rowData, props.rowField)" v-if="User.accessMenuDB[1].children[1].children[1].selected === true">
        ...
        <div class="custom-actions" v-if="props.rowData.Actived == 1">
          <b-btn class="btn btn-outline-success btn-sm mr-1 mt-1"
            @click="onAction('publish-item', props.rowData, props.rowIndex)" v-if="User.accessMenuDB[1].children[1].children[5].selected === true">
        ...
        <div class="custom-actions" v-if="props.rowData.Actived == 2">
          ...
          <div v-if="props.rowData.StatusApproved == 2">
            <a class="btn btn-xs btn-outline-danger text-danger" :href="'/AdminVue/auditor-team/'+props.rowData.id+'/exportDaftarHadir'" target="_blank" v-if="User.accessMenuDB[1].children[1].children[4].selected === true"> <i class="fas fa-print"></i> Export Daftar Hadir</a>
          </div>
        </div>
      </template>

      <template slot="Workflow" slot-scope="props">
          <div v-if="props.rowData.Actived == 1">
            Belum Tersubmit
          </div>
          <div v-else-if="props.rowData.Actived == 2 && props.rowData.StatusApproved == 1">
            Submit
          </div>
          <div v-else-if="props.rowData.Actived == 2 && props.rowData.StatusApproved == 2">
            Submit
            <b-btn class="btn btn-outline-danger btn-sm mr-1 mt-1"
            @click="onAction('close-item', props.rowData, props.rowIndex)" v-if="props.rowData.IsReport != 1 && props.rowData.CekAuditor == true">
