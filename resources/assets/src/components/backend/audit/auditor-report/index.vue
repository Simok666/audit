<template> <div>
  ...
  <b-card header="View / Data Audit Report" header-tag="h4" class="mb-4">
      ...
      <div class="col-md-6">
          ...
          <b-btn @click="createData()" class="btn btn-success btn-md float-right mr-2" v-if="User.accessMenuDB[1].children[2].children[0].selected === true">
      ...
      <template slot="tableHeader">
        <tr class="text-center">
          <th class="wd-50"></th>
          <th>
            <b-input v-model="paramData.search.adr__NoTrans" placeholder="IdAudit" v-on:keyup.enter="getFilters()" />
          </th>
          <th>
          ...
          <th>
            <masked-input type="text" class="form-control" placeholder="Thn-Bln-Tgl"
              v-model="paramData.search.adr__AuditorDate"
          ...
          <th></th>
          <th>
            <b-input v-model="paramData.search.adr__PotentialNonConformitiy" placeholder="Potential Non Conformity" v-on:keyup.enter="getFilters()" />
          </th>
          <th>
            <b-input v-model="paramData.search.adr__TypeNonConformity" placeholder="Type Non Conformity" v-on:keyup.enter="getFilters()" />
          </th>
          <th></th>
          <th></th>
          <th></th>
      ...
      <template slot="Path" slot-scope="props">
          <div v-if="props.rowData.Path != ''">
            ...
            @click="viewAttachment(props.rowData, props.rowIndex)" v-if="User.accessMenuDB[1].children[2].children[2].selected === true">
      ...
      <template slot="Clause" slot-scope="props">
        ...
        <template slot="Actions" slot-scope="props">
            <div class="custom-actions" v-if="props.rowData.Actived == 1">
                ...
                @click="onAction('edit-item', props.rowData, props.rowIndex,0)" v-if="User.accessMenuDB[1].children[2].children[0].selected === true && props.rowData.CekAuditor == true">
                <i class="ion ion-md-create"></i> Edit
                </b-btn>
                <b-btn class="btn btn-outline-danger btn-sm mr-1 mt-1"
                @click="onAction('delete-item', props.rowData, props.rowField,0)" v-if="User.accessMenuDB[1].children[2].children[1].selected === true && props.rowData.CekAuditor == true">
        ...
        <div class="custom-actions" v-if="props.rowData.Actived == 1">
          <b-btn class="btn btn-outline-success btn-sm mr-1 mt-1"
            @click="onAction('publish-item', props.rowData, props.rowIndex,0)" v-if="User.accessMenuDB[1].children[2].children[4].selected === true && props.rowData.CekAuditor == true">
        ...
        <div class="custom-actions" v-if="props.rowData.Actived == 2 && props.rowData.StatusApproved == 2">
          <a class="btn btn-xs btn-outline-danger text-danger" :href="'/AdminVue/auditor-report/'+props.rowData.id+'/exportStatusAudit'" target="_blank" v-if="User.accessMenuDB[1].children[2].children[3].selected === true"> <i class="fas fa-print"></i> Export Status Audit</a>
          <b-btn class="btn btn-outline-danger btn-sm mr-1 mt-1"
            @click="onAction('close-item', props.rowData, props.rowIndex)" v-if="props.rowData.IsCapaPlan != 1 && props.rowData.CekAuditor == true">
            <i class="ion ion-md-close"></i> Close Data
            </b-btn>
        </div>
      ...
      <template slot="Workflow" slot-scope="props">
          <div v-if="props.rowData.ApprovedOrdinal == 0 && props.rowData.StatusApproved == 1">
