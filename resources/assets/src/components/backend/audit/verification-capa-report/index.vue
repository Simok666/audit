<template> <div>
  ...
  <b-card header="View / Data Verification Capa Report" header-tag="h4" class="mb-4">
      ...
      <div class="col-md-6">
          ...
          <b-btn @click="createData()" class="btn btn-success btn-md float-right mr-2" v-if="User.accessMenuDB[1].children[4].children[0].selected === true">
      ...
      <template slot="tableHeader">
        <tr class="text-center">
          <th class="wd-50"></th>
          <th>
            <b-input v-model="paramData.search.acv__NoTrans" placeholder="IdAudit" v-on:keyup.enter="getFilters()" />
          </th>
          <th>
          ...
          <th>
            <masked-input type="text" class="form-control" placeholder="Thn-Bln-Tgl"
              v-model="paramData.search.acv__AuditorDate"
          ...
          <th></th>
          <th>
            <b-input v-model="paramData.search.acv__PotentialNonConformitiy" placeholder="Potential Non Conformity" v-on:keyup.enter="getFilters()" />
          </th>
          <th>
            <b-input v-model="paramData.search.acv__TypeNonConformity" placeholder="Type Non Conformity" v-on:keyup.enter="getFilters()" />
          </th>
          <th></th>
          <th></th>
          <th></th>
      ...
      <template slot="Clause" slot-scope="props">
       ...
       <template slot="Path" slot-scope="props">
          <div class="custom-path">
            ...
            @click="viewAttachment(props.rowData, props.rowIndex)">
        ...
        <template slot="Actions" slot-scope="props">
            <div class="custom-actions" v-if="props.rowData.Actived == 1 && User.IdEmployee == props.rowData.IdEmployeAuditee">
                ...
                @click="onAction('edit-item', props.rowData, props.rowIndex,0)" v-if="User.accessMenuDB[1].children[4].children[0].selected === true">
                <i class="ion ion-md-create"></i> Edit
                </b-btn>
                <b-btn class="btn btn-outline-danger btn-sm mr-1 mt-1"
                @click="onAction('delete-item', props.rowData, props.rowField,0)" v-if="User.accessMenuDB[1].children[4].children[1].selected === true">
        ...
        <div class="custom-actions" v-if="props.rowData.Actived == 1 && User.IdEmployee == props.rowData.IdEmployeAuditee">
          <b-btn class="btn btn-outline-success btn-sm mr-1 mt-1"
            @click="onAction('publish-item', props.rowData, props.rowIndex,0)" v-if="User.accessMenuDB[1].children[4].children[3].selected === true">
            <i class="ion ion-md-checkmark"></i> Send to Workflow
          </b-btn>
        </div>
        <div class="custom-actions" v-if="props.rowData.ApprovedOrdinal == 1 && props.rowData.IdVerificationExecution == 0 && props.rowData.IdEmployeAuditor == User.IdEmployee">
          <b-btn class="btn btn-outline-secondary btn-sm mr-sm-1"
            @click="onAction('revisi-item', props.rowData, props.rowIndex,0)" v-if="User.accessMenuDB[1].children[4].children[0].selected === true">
            <i class="ion ion-md-create"></i> Verification
          </b-btn>
        </div>
        <div class="custom-actions" v-if="props.rowData.ApprovedOrdinal == 1 && props.rowData.IdVerificationExecution != 0 && props.rowData.IdEmployeAuditor == User.IdEmployee">
          <b-btn class="btn btn-outline-success btn-sm mr-1 mt-1"
            @click="onAction('publish-item-revisi', props.rowData, props.rowIndex,0)" v-if="User.accessMenuDB[1].children[4].children[3].selected === true">
            <i class="ion ion-md-checkmark"></i> Send to Workflow
          </b-btn>
          <b-btn class="btn btn-outline-secondary btn-sm mr-sm-1"
            @click="onAction('edit-revisi-item', props.rowData, props.rowIndex,0)" v-if="User.accessMenuDB[1].children[4].children[0].selected === true">
            <i class="ion ion-md-create"></i> Edit Verification
          </b-btn>
        </div>
        <div class="custom-actions" v-if="props.rowData.Actived == 2 && props.rowData.StatusApproved == 2">
          <a class="btn btn-xs btn-outline-danger text-danger" :href="'/AdminVue/verification-capa-report/'+props.rowData.id+'/exportHasilAudit'" target="_blank" v-if="User.accessMenuDB[1].children[4].children[2].selected === true"> <i class="fas fa-print"></i> Export Hasil Audit</a>
        </div>
        <!-- <div class="custom-actions" v-if="props.rowData.Actived == 2 && props.rowData.StatusApproved == 1">
          <div v-if="props.rowData.ApprovedOrdinal == 0 && props.rowData.ApprovedEmployee == IdEmployee">
            <b-btn class="btn btn-outline-success btn-sm mr-1 mt-1"
              @click="onAction('approve-item', props.rowData, props.rowIndex,0)">
              <i class="ion ion-ios-create"></i> Approve
            </b-btn>
          </div>
          <div v-else-if="props.rowData.ApprovedOrdinal == 1 && props.rowData.ApprovedEmployee == IdEmployee">
            <b-btn class="btn btn-outline-success btn-sm mr-1 mt-1"
              @click="onAction('approve-item', props.rowData, props.rowIndex,1)">
              <i class="ion ion-ios-create"></i> Approve
            </b-btn>
            <b-btn class="btn btn-outline-danger btn-sm mr-sm-1 mt-1"
                @click="onAction('reject-item', props.rowData, props.rowIndex,1)">
                <i class="ion ion-md-warning"></i> Tolak
            </b-btn>
          </div>
          <div v-else-if="props.rowData.ApprovedOrdinal == 2 && props.rowData.ApprovedEmployee == IdEmployee">
            <b-btn class="btn btn-outline-success btn-sm mr-1 mt-1"
              @click="onAction('approve-item', props.rowData, props.rowIndex,2)">
              <i class="ion ion-ios-create"></i> Approve
            </b-btn>
            <b-btn class="btn btn-outline-danger btn-sm mr-sm-1 mt-1"
                @click="onAction('reject-item', props.rowData, props.rowIndex,2)">
                <i class="ion ion-md-warning"></i> Tolak
            </b-btn>
          </div>
        </div> -->
