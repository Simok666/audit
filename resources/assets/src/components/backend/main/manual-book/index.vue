<template> <div>
  ...
  <b-card header="View / Data Manual Book" header-tag="h4" class="mb-4">
      ...
      <div class="col-md-6">
          ...
          <b-btn @click="createData()" class="btn btn-success btn-md float-right mr-2" v-if="User.accessMenuDB[3].children[0].selected === true">
      ...
      <template slot="tableHeader">
        <tr class="text-center">
          <th class="wd-50"></th>
          <th>
            <b-input v-model="paramData.search.usr__UserName" placeholder="Nama Karyawan" v-on:keyup.enter="getFilters()" />
          </th>
          <th></th>
          <th></th>
          <th></th>
      ...
      <template slot="Document" slot-scope="props">
          {{ getNameFile(props.rowData.Path) }}
      </template>
      ...
      <template slot="Actions" slot-scope="props">
            <div class="custom-actions">
                <a class="btn btn-xs btn-outline-success text-success mr-sm-1" :href="BaseUrl+props.rowData.Path" target="_blank"> <i class="fas fa-download"></i> Download</a>
                <b-btn class="btn btn-outline-secondary btn-sm mr-sm-1"
                @click="onAction('edit-item', props.rowData, props.rowIndex)" v-if="User.accessMenuDB[3].children[0].selected === true">
                <i class="ion ion-md-create"></i> Edit
                </b-btn>
                <b-btn class="btn btn-outline-danger btn-sm mr-1 mt-1"
                @click="onAction('delete-item', props.rowData, props.rowField)" v-if="User.accessMenuDB[3].children[1].selected === true">
...
export default {
  ...
  methods: {
    ...
    getNameFile(data){
        let NameFile = data.replace('clouds/backend/files/documents/','')
        return NameFile;
    },
