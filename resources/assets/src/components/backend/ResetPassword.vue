    ...
    <div class="authentication-inner py-5">
      ...
      <b-card no-body>
        <div class="p-4 p-sm-5">
          ...
          <h2 class="text-center font-weight-bold mb-4 text-danger" >Reset Password</h2>

          <!-- Form -->
          <form @submit.prevent="ResetPassword()">
            <b-form-group label="Password Baru">
              <b-input type="password" ref="newpassword" v-model="credentials.newpassword" required />
            </b-form-group>
            <b-form-group label="Ulangi Password Baru">
              <b-input type="password" ref="newpasswordrepeat" v-model="credentials.newpasswordrepeat" required />
            </b-form-group>
            <!-- <b-form-group>
            ...
            <div class="">
              ...
              <b-btn type="submit" variant="primary" style="width:100%">Reset Password</b-btn>
            </div>
          </form>
          <!-- / Form -->
        </div>
...
export default {
  ...
  methods: {
    ResetPassword() {
      ...
      this.showLoading()
      axios.post('/AdminVue/reset-password', data)
        .then(({data}) => {
          this.$router.push({
            name: 'main/login',
            params: {
        ...
        .catch(({response}) => {
          this.$Progress.fail()
          setTimeout(() => this.$swal('Reset Password', response.data.message), 500)
          this.hideLoading()
        })
    }
  },
